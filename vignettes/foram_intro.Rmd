...
```{r}
age_index <- age_index(load_proxy = load_foram, age_units = 'kyr', step_type = 'every')
```
...
The user-changeable parameters are listed in the helkp documentation of this function. For 'change_values' argument, provide a vector of numeric values which correspond to the parameter names in 'parms2change'. To demonstrate, we will adjust the temperature and d11Bsw prior distribution parameters.
```{r}
...
priors_foram <- priors_foram(parms_foram_adj = parms_foram_adj, age_index = age_index, cc2ndparm.vt = 'dic', cc2ndparm.pt = 'ts', 
                             cc2ndparmTS = dic_pri)
```
